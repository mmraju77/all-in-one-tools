...
import { createClient } from "@/lib/supabase"; // 👈 FIX: Using your project's secure cookie-aware client
import { 
...
export default function ToolWorkspace({ toolName = "Tool", slug = "", category = "Category" }: { toolName?: string, slug?: string, category?: string }) {
  ...
  useEffect(() => {
    const verifyProStatus = async () => {
      try {
        // Initialize with the secure cookie-aware client from your app
        ...
        if (user && user.email) {
          ...
          if (data && (data.isPro === true || data.isPro === "true")) {
            setUserIsPro(true); // Unlock instantly
          }
        }
      } catch (err) {
